...
DefinitionBlock ("", "DSDT", 2, "_ASUS_", "Notebook", 0x00000219)
{
    ...
    External (_SB_.PCI0.IGPU.NXTD, IntObj)
...
P8XH (One, 0xAB)
    ...
    Scope (_SB)
    {
        ...
        Device (ATKD)
        {
            ...
            Method (SDSP, 1, NotSerialized)
            {
                If (LEqual (One, FNF8))
                {
                    Store (0x0400, ^^PCI0.IGPU.NADL)
                    Store (Zero, ^^PCI0.IGPU.NDL2)
                    Store (Zero, ^^PCI0.IGPU.NDL3)
                    ^^PCI0.IGPU.GNOT (One, 0x80)
                }

                If (LEqual (0x02, FNF8))
                {
                    Store (0x0100, ^^PCI0.IGPU.NADL)
                    Store (Zero, ^^PCI0.IGPU.NDL2)
                    Store (Zero, ^^PCI0.IGPU.NDL3)
                    ^^PCI0.IGPU.GNOT (One, 0x80)
                }
    ...
    Scope (_SB.PCI0.LPCB)
    {
        Device (EC0)
        {
            ...
            Method (_REG, 2, NotSerialized)  // _REG: Region Availability
            {
                ...
                If (LAnd (LEqual (Arg0, 0x03), LEqual (Arg1, One)))   //add  disable  one
                {
                    ECNV (0x02)
                }
    ...
    Scope (_SB.PCI0.LPCB.EC0)
    {
        ...
        Method (_Q19, 0, Serialized)  // _Qxx: EC Query
        {
            ...
            If (LEqual (FNF8, Zero))
            {
                Or (^^^IGPU.CADL, ^^^IGPU.CAL2, Local0)
                While (One)
                {
                    Store (Local0, _T_0)
                    If (LEqual (_T_0, 0x0100))
                    {
                        Store (0x02, ^^^IGPU.NXTD)
                    }
                    ElseIf (LEqual (_T_0, 0x0300))
                    {
                        If (LEqual (^^^IGPU.CADL, 0x0100))
                        {
                            Store (Zero, ^^^IGPU.NXTD)
                        }
                        Else
                        {
                            Store (0x04, ^^^IGPU.NXTD)
                        }
                    }
                    ...
                    {
                        Store (0x03, ^^^IGPU.NXTD)
                    }
                    ElseIf (LEqual (_T_0, 0x0700))
                    {
                        Store (0x05, ^^^IGPU.NXTD)
                    }
            ...
            If (ATKP)
            {
                Store (LEqual (LOr (LOr (LEqual (^^^IGPU.CPDL, 0x0300), LEqual (^^^IGPU.CPL2, 0x0300)), LEqual (^^^IGPU.CPL3, 0x0300)), Zero), HDMI)
                If (LNotEqual (HDMI, Zero))
                {
                    If (LEqual (^^^IGPU.CPDL, 0x0400))
                    {
                        Store (Zero, ^^^IGPU.NXTD)
                    }

                    If (LEqual (^^^IGPU.CPDL, 0x0100))
                    {
                        If (LGreaterEqual (^^^IGPU.NXTD, 0x03))
                        {
                            Store (Zero, ^^^IGPU.NXTD)
                        }
                    }
                ...
                If (LEqual (HDMI, Zero))
                {
                    If (LEqual (^^^IGPU.CPDL, 0x0400))
                    {
                        If (LEqual (^^^IGPU.NXTD, One))
                        {
                            Store (0x03, ^^^IGPU.NXTD)
                        }

                        If (LEqual (^^^IGPU.NXTD, 0x02))
                        {
                            Store (0x03, ^^^IGPU.NXTD)
                        }

                        If (LEqual (^^^IGPU.NXTD, 0x05))
                        {
                            Store (Zero, ^^^IGPU.NXTD)
                        }
                    }
                ...
                If (LEqual (^^^IGPU.NXTD, 0x06))
                {
                    Store (Zero, ^^^IGPU.NXTD)
                }

                If (LEqual (^^^IGPU.NXTD, Zero))
                {
                    ^^^^ATKD.IANE (0x61)
                    Store (One, FNF8)
                }
                ...
                If (LEqual (HDMI, Zero))
                {
                    If (LEqual (^^^IGPU.NXTD, 0x03))
                    {
                        ^^^^ATKD.IANE (0xA0)
                        Store (0x04, FNF8)
                    }

                    If (LEqual (^^^IGPU.NXTD, 0x04))
                    {
                        ^^^^ATKD.IANE (0xA1)
                        Store (0x05, FNF8)
                    }
